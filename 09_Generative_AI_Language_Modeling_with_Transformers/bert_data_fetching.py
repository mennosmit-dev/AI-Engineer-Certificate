...
"""Data Preparations for BERT.ipynb
...
tokenizer = BertTokeniser.from_pretrained('bert-base-uncased')
...
def yield_tokens(data_iter):
    for _, el in data_iter:
        tokens = tokenizer(el, return_tensors='pt', truncation=True, max_length=1000)['input_ids'][0]  # The pt here means return as python tensors.
        yield tokens.tolist()
...
""" If you want to use your own tokeniser it is done like this:
special_symbols = ['[PAD]', '[CLS]', '[SEP]', '[MASK]', '[UNK]']
special_indices = [0, 1, 2, 3, 4]  # Alligns with line above

train_iter, test_iter = IMBD(split=('train', 'test'))

vocab = build_vocab_from_iterator(yield_tokens(train_iter)), specials = special_symbols)
vocab.set_default_index(4)
"""
