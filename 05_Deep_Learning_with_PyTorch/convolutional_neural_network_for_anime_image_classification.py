...
Original file is located at
    https://colab.research.google.com/drive/18_4gSL--fM_L9Prwz1-oZfZl9-Jouz8y
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)) #this normalises the pixels which makes CNN learn faster
])
...
class AnimeCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, 3, 1, padding=1) #32 feature maps, first 64x64 activation maps, after maxpooling 32x32
        self.conv2 = nn.Conv2d(32, 64, 3, 1, padding=1) #so 64 feature maps, first all of 32x32 and after pooling 16x16
        self.pool = nn.MaxPool2d(2, 2) #decreases the dimension by 1/2
...
input_tensor = torch.randn(1, 3, 64, 64) #amount of images, amount of in dimensions CNN, pixels
...
def imshow(img, ax):
    img = img / 2 + 0.5  # unnormalize, such that in [0,1]
    npimg = img.numpy()
...
fig, axs = plt.subplots(num_rows, num_cols * 2, figsize=(20, num_rows)) #times two to also show labels
...
num_epochs = 20 # increased from 7 to 20 but didnt really matter that much anymore after epoch 7
train_losses = []
val_losses = []
...
#from here all the same as earlier
